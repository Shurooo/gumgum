...
def crawl(addr_io):
    ...
    data_new = get_data(os.path.join(addr_day_out, "day_samp_new_pos.npy"), 25000)

    data = vstack([data_old, data_new])
    np.random.shuffle(data)

    with open(os.path.join(addr_day_out, "day_samp_res_pos_25.npy"), "w") as file_out:
