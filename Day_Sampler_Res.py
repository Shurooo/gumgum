...
def crawl(addr_io):
    ...
    data_new = get_data(os.path.join(addr_day_out, "day_samp_neg_pos.npy"), 75000)

    data = vstack([data_old, data_new])
    np.random.shuffle(data)

    with open(os.path.join(addr_day_out, "day_samp_res_neg_75.npy"), "w") as file_out:
