...
def train(addr_train, clf, sampling, onoff_line):
    path_in = os.path.join(addr_train, "day_samp_num.npy")
    ...
    if __SAVE_MODEL:
        model_name = "MNB_" + onoff_line +"_" + sampling + "_Model"
...
def init_clf(sampling):
    clf_options = {
        "None": (MultinomialNB(class_prior=[0.05, 0.95]), "cp=[0.05 0.95]"),
        "Over": (MultinomialNB(class_prior=[0.01, 0.99]), "cp=[0.01 0.99]; ratio={}".format(__RATIO_OVER)),
        "Under": (MultinomialNB(class_prior=[0.01, 0.99]), "cp=[0.01 0.99]; ratio={}".format(__RATIO_UNDER))
    }
